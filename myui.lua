...
function library:Unload()
    ...
    for _, o in next, self.options do
        if o.type == "toggle" then
            coroutine.resume(coroutine.create(o.SetState))
        end
    end
    library = nil
    getgenv().library = nil
...
library.createSlider = function(option, parent)
    ...
    function option:SetValue(value, nocallback)
        ...
        if self.min >= 0 then
            local targetSize = UDim2.new((value - self.min) / (self.max - self.min), 0, 1, 0)
            local tweenInfo = TweenInfo.new(0.05, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
            local tween = TweenService:Create(option.fill, tweenInfo, {Size = targetSize})
            tween:Play()
        else
            option.fill:TweenPosition(UDim2.new((0 - self.min) / (self.max - self.min), 0, 0, 0), "Out", "Quad", 0.05, true)
            local targetSize = UDim2.new(value / (self.max - self.min), 0, 1, 0)
